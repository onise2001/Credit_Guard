...
class TestCardModel(APITestCase):
    ...
    def setUp(self):
        ...
        self.list_create_url = reverse('cardsviewset')
    ...
    def test_serializer_validation(self):
        ...
        elapsed_time = 0
        card = { 'title': "Test card validation"}
        for index in range(len(card_numbers)):
            card['card_number'] = card_numbers[index]
            card['ccv'] = ccvs[index]
            start_time = time.time()
            response = self.client.post(self.list_create_url, card,format='json')
            end_time = time.time()
